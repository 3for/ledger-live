...
export async function runInlineAddAccountTest(
  ...
  describe("Inline Add Account - LLM", () => {
    beforeAll(async () => {
...
export async function runStartETHStakingFromEarnDashboardTest(
  ...
  describe("Start ETH staking flow from Earn Dashboard - LLM", () => {
    beforeAll(async () => {
      await beforeAllFunction({
        userdata: "skip-onboarding",
        speculosApp: account.currency.speculosApp,
    ...
    tmsLinks.forEach(tmsLink => $TmsLink(tmsLink));
    tags.forEach(tag => $Tag(tag));
    it(`ETH staking flow - Earn Dashboard - ${account.currency.speculosApp.name}`, async () => {
...
export async function runCorrectEarnPageIsLoadedDependingOnUserStakingSituationTest(
  ...
  describe("Correct Earn page is loaded depending on user's staking situation - LLM", () => {
    beforeAll(async () => {
      await beforeAllFunction({
        userdata: "skip-onboarding",
        speculosApp: account.currency.speculosApp,
