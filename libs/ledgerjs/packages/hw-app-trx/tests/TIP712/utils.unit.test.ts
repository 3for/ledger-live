    ...
    describe("TIP712_TYPE_ENCODERS", () => {
      describe("INT", () => {
          ...
          couples.forEach(([value, sizes]) => {
            sizes.forEach(size => {
              test(`should have the right return for value: ${value} and size: ${size}`, () => {
                const hexBuffer = TIP712_TYPE_ENCODERS.INT(value, size);
        ...
        describe("from hex string", () => {
          couples.forEach(([value, sizes]) => {
            sizes.forEach(size => {
              test(`should have the right return for value: ${value} and size: ${size}`, () => {
                const valueAsHex = TIP712_TYPE_ENCODERS.INT(value, size).toString("hex"); // Reusing valid values from a first run

                const hexBuffer = TIP712_TYPE_ENCODERS.INT("0x" + padHexString(valueAsHex), size);
      ...
      describe("UINT", () => {
        it("should use the same logic as INT", () => {
          ...
          const spy = jest.spyOn(TIP712_TYPE_ENCODERS, "INT");
          TIP712_TYPE_ENCODERS.UINT(value);
      ...
      describe("BOOL", () => {
        test("should not break from null", () => {
          expect(TIP712_TYPE_ENCODERS.BOOL(null).toString("hex")).toEqual("00");
        });

        describe("from string", () => {
          test("should return 00", () => {
            expect(TIP712_TYPE_ENCODERS.BOOL("0").toString("hex")).toEqual("00");
          });

          test("should return 00", () => {
            expect(TIP712_TYPE_ENCODERS.BOOL("0x00").toString("hex")).toEqual("00");
          });

          test("should return 01", () => {
            expect(TIP712_TYPE_ENCODERS.BOOL("1").toString("hex")).toEqual("01");
          });

          test("should return 01", () => {
            expect(TIP712_TYPE_ENCODERS.BOOL("0x01").toString("hex")).toEqual("01");
          });
        });

        describe("from boolean", () => {
          test("should return 00", () => {
            expect(TIP712_TYPE_ENCODERS.BOOL(false).toString("hex")).toEqual("00");
          });

          test("should return 01", () => {
            expect(TIP712_TYPE_ENCODERS.BOOL(true).toString("hex")).toEqual("01");
          });
        });
      ...
      describe("ADDRESS", () => {
        test("should not break from null", () => {
          expect(TIP712_TYPE_ENCODERS.ADDRESS(null).toString("hex")).toEqual("");
        });

        test("should return 183c938611642ae18790db0A1eFC21Dfe009aA1c", () => {
          expect(
            TIP712_TYPE_ENCODERS.ADDRESS("0x183c938611642ae18790db0A1eFC21Dfe009aA1c").toString(
              "hex",
            ),
      ...
      describe("STRING", () => {
        test("should not break from null", () => {
          expect(TIP712_TYPE_ENCODERS.STRING(null).toString("hex")).toEqual("");
        });

        test("should not break from empty string", () => {
          expect(TIP712_TYPE_ENCODERS.STRING("").toString("hex")).toEqual("");
        });

        test("should return 6b766e49734147656e697573", () => {
          expect(TIP712_TYPE_ENCODERS.STRING("kvnIsAGenius").toString("hex")).toEqual(
      ...
      describe("BYTES", () => {
        describe("from fixed size bytes", () => {
          test("should not break from null", () => {
            expect(TIP712_TYPE_ENCODERS.BYTES(null).toString("hex")).toEqual("");
          });

          test("should return 973bb640", () => {
            expect(TIP712_TYPE_ENCODERS.BYTES("0x973bb640", 4).toString("hex")).toEqual("973bb640");
          });
          test("should return 973bb640", () => {
            expect(
              TIP712_TYPE_ENCODERS.BYTES(
